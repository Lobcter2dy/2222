#!/usr/bin/env python3
"""
Элемент: Панель (Panel)
Прямоугольный элемент с заливкой
Используется как контейнер или фон
"""
from ..element_base import ElementBase


class PanelElement(ElementBase):
    """Панель - прямоугольник с заливкой"""
    ...
    def __init__(self, canvas, config):
        ...
        # Настройки по умолчанию для панели
        self.properties.update({
            'fill_color': '#3a3a3a',     # Серая заливка
            'stroke_color': '#5a5a5a',   # Светлая обводка
            'stroke_width': 1,
            'display_mode': 'both',       # Обводка + заливка
            
            # Специфичные для панели
            'panel_function_id': 0,      # ID привязанной функции
            'spawn_points': [],          # Точки появления
        })
    ...
    def draw(self):
        ...
        # 4. Внутренняя тень
        if self.properties['inset_shadow']:
        ...
        # 6. Индикатор функции
        if self.properties.get('panel_function_id', 0) > 0:
    ...
    def _draw_function_indicator(self, x1, y1):
        ...
        func_id = self.properties.get('panel_function_id', 0)
        if func_id <= 0:
            return
        ...
        # Фон индикатора (оранжевый для панели)
    ...
    def get_function_id(self):
        ...
        return self.properties.get('panel_function_id', 0)

    def set_function_id(self, func_id):
        """Устанавливает ID функции"""
        self.properties['panel_function_id'] = func_id
        self.update()
